...
export default function ProductPage({ params: { locale, slug } }) {
  ...
  return (
    <div>
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-white/80 text-xl font-semibold mb-5">{pt.tagline}</p>
              <p className="text-white/70 text-base leading-relaxed mb-8">{pt.description}</p>
              <a href="#contact"
            ...
            <div className="lg:pl-4">
              <div className="rounded-2xl overflow-hidden shadow-2xl shadow-black/30 border border-white/10">
                <div className="bg-[#0d1f38]/80 px-4 py-2.5 flex items-center gap-2 border-b border-white/10">
                  ...
                  <span className="ms-2 text-[10px] text-white/40 font-mono">{p.name} — Live</span>
                </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {p.stats.map((s, i) => (
              <div key={i} className="text-center p-6 rounded-2xl border border-gray-100">
                ...
                <div className="text-sm text-gray-500 font-medium">{pt.statLabels[i]}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-gray-50 py-16">
        <div className="max-w-5xl mx-auto px-4">
          <h2 className="text-xl font-black text-gray-900 mb-8">{t('exploreOther')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {otherProducts.map(op => {
              const OtherIcon = ICON_MAP[op.lucideIcon] || Building;
              const opt = tp.raw(op.slug);
              return (
                <Link key={op.slug} href={`/solutions/${op.slug}`}
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 font-medium truncate">{opt.tagline}</div>
                  </div>
